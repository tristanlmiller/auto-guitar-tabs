#!/usr/bin/env python
'''
Trains a extreme gradient boosting classifier on processed data, saves model as pickle.

usage: source destination [--weighted --num num_estimators --depth max_depth --learn learning_rate --frac fraction --metrics]
...
--learn learning_rate - Learning rate of the XGB model
...
def main():
    ...
    if args[0] == '--metrics':
        metrics_only = True
        del args[0:1]
    train(source, destination, source_dir, target_dir, weight, num_estimators, max_depth, learning_rate, fraction, metrics_only)
    
def train(source, destination, source_dir, target_dir, weight, num_estimators, max_depth, learning_rate, fraction, metrics_only):
    ...
    if not metrics_only:
        ...
        if weight == 'balanced':
            ...
            if curr_data_info['standard']:
                quality_model.fit(standard_features_train, standard_labels_train[:,1],sample_weight = get_weights(standard_labels_train[:,1]))
                add_model.fit(standard_features_train, standard_labels_train[:,2],sample_weight = get_weights(standard_labels_train[:,2]))
                inv_model.fit(standard_features_train, standard_labels_train[:,3],sample_weight = get_weights(standard_labels_train[:,3]))
            else:
                quality_model.fit(features_train, labels_train[:,1],sample_weight = get_weights(labels_train[:,1]))
                add_model.fit(features_train, labels_train[:,2],sample_weight = get_weights(labels_train[:,2]))
                inv_model.fit(features_train, labels_train[:,3],sample_weight = get_weights(labels_train[:,3]))
        else:
        ...
        if not os.path.exists(target_dir + 'xgb_directory.csv'):
            header = 'sourcepath,filepath,weighted,fraction,num_estimators,max_depth,learning_rate,Model'
            with open(target_dir + 'xgb_directory.csv','a') as f:
                f.write(header)

        #record settings in file
        newrow = f"\n{source},{destination},{weight},{fraction},{num_estimators},{max_depth},{learning_rate},xgb"
        with open(target_dir + 'xgb_directory.csv','a') as f:
            f.write(newrow)
    else:
        with open(f'{target_dir}{destination}.pkl', 'wb') as f:
